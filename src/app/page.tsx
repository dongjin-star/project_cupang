...
import type { ConfirmedInfo, CostInfo, Listing } from '@/types/listing'
    ...
    <div className="mx-auto w-full max-w-6xl px-4 pb-32 pt-6 sm:px-6 lg:pb-10">
      <header className="mb-6">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted">
          도매 상품 정보를 넣으면 WING 폼 순서 그대로 14개 섹션의 입력값을 만들어 드립니다.
          마진이 음수인 가격과 정책 위반 문구는 등록 전에 걸러집니다.
        </p>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
        <form onSubmit={submit} className="space-y-4">
          <FormSection
            title="상품 정보"
            description="AI가 잘못 읽을 여지가 없도록, 실제 상품 그대로 입력하세요."
          >
            ...
            <TextField
              ...
              required
            />
            <TextField label="색상" value={color} onChange={setColor} placeholder="투명" />
            <TextField label="원산지" value={origin} onChange={setOrigin} placeholder="중국" />
            <NumberField
              label="단위 수량"
              value={qtyPerUnit}
          ...
          <FormSection title="크기" description="검색필터 치수와 고시정보 크기에 그대로 쓰입니다.">
            <NumberField label="지름" value={diameter} onChange={setDiameter} suffix="cm" hint="원형 상품만" />
            <NumberField label="높이" value={height} onChange={setHeight} suffix="cm" hint="필수" />
            <NumberField label="가로" value={width} onChange={setWidth} suffix="cm" hint="사각형 상품" />
            <NumberField label="세로" value={depth} onChange={setDepth} suffix="cm" hint="사각형 상품" />
          </FormSection>

          <FormSection title="원가·가격" description="마진 계산기가 우측(모바일은 하단)에서 즉시 재계산합니다.">
            <NumberField label="도매 단가" value={unitPrice} onChange={setUnitPrice} suffix="원" />
            <NumberField
              label="입고 배송비"
          ...
          >
            <TextField label="도매 사이트" value={sourceSite} onChange={setSourceSite} />
            <TextField
              label="도매 상품번호"
              value={sourceProductNo}
              onChange={setSourceProductNo}
